...
void TPM0_Init_PWM(void) {
	...
	PORTB->PCR[2] = PORT_PCR_MUX(2);		// ToDo 3.1.2: Set multiplekser to TPM0 for PTB9, get channel number (page 148 of the Reference Manual)

	TPM0->SC |= TPM_SC_PS(128);  					// ToDo 3.1.3: Set prescaler to 128
	TPM0->SC |= TPM_SC_CMOD(1);					// ToDo 3.1.4: For TMP0, select the internal input clock source

	TPM0->MOD = 98; 										// ToDo 3.1.5: Set modulo value to maximum value from slider
...
// ToDo 3.1.6: Connect correct channel from TPM0 for "Edge-aligned PWM Low-true pulses" mode
	...
	TPM0->CONTROLS[0].CnSC |= (TPM_CnSC_MSB_MASK | TPM_CnSC_ELSA_MASK); /* set Output on match, clear Output on reload */ 

	TPM0->CONTROLS[0].CnV = 50; 				// ToDo 3.1.7: Set starting value to 50
