...
\******************************************************************************/
	...
	I2C_WriteReg(0x1A, 0x2A, 0x01);	 		// Set ACTIVE bit in CTRL_REG1 in MMA8451

	I2C_ReadReg (0x1A, 0x2A, &readout); // Get OUT_Z_MSB from MMA8451 (MSB 8 bits from 14 in total)
	sprintf(temp,"Gravity: %+1.2fg", ((double)((int16_t)(readout<<8)>>2)/1024)); // default 4096 counts/g sensitivity
...
\******************************************************************************/	
	...
	I2C_ReadRegBlock(0x1A, 0x01, 6, arrayXYZ);

	sprintf(temp,"X: %+1.6fg", ((double)((int16_t)((arrayXYZ[0]<<8)|arrayXYZ[1])>>2)/1024)); // default 4096 counts/g sensitivity
	UART_Print(temp);

	sprintf(temp,"Y: %+1.6fg", ((double)((int16_t)((arrayXYZ[2]<<8)|arrayXYZ[3])>>2)/1024)); // default 4096 counts/g sensitivity
	UART_Print(temp);

	sprintf(temp,"Z: %+1.6fg", ((double)((int16_t)((arrayXYZ[4]<<8)|arrayXYZ[5])>>2)/1024)); // default 4096 counts/g sensitivity
	UART_Print(temp);
